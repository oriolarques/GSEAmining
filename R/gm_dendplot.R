...
#' the clusters to help differentiating them.
#'
#' @return
#' @export
#'
...
gm_dendplot <- function(df,
  ...
  # Create an object rom geneset table
  #   and assign the color depending if NES < | > 0
  dend_label_col <- df %>%
    select(ID, NES) %>%
  ...
  # Color of the clusters: Red=positively enriched / Blue = negatively enriched
